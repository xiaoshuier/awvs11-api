        ...
        for parameter in parameters:
            if parameter.getType() in [0, 1]:
                for clrf in _payloads:
                    ...
                    num=fuzz.ratio(first_response_bodys,urllib.quote(new_response_bodys))
                    print '相似度为：%s' % num
                    if num <= 97: #加入payload后如果页面相似度小于%96，那就进行下一步判断
                        #print(clrf[1])
                        newRequest = self._helpers.removeParameter(messageInfo.getRequest(), parameter) #除去全部参数的值
                        ...
                        if fuzz.ratio(first_response_bodys,urllib.quote(new_response_bodys)) > 97: #如果相似度大于96那就证明是存在sql注入的
                            print "sqli##############################################################sqli"
                            issue=CustomScanIssue(httpService, url,
                                                               [attack],
                                                               'SQL injection',
